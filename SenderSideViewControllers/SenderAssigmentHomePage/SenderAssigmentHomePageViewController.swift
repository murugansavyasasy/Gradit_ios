...
class SenderAssigmentHomePageViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if assigmentSegmentName.selectedSegmentIndex == 0{
            ...
            let  plusoneCount  = past.newfilepath?.count ?? 0-1
...
func apread(gesture : String){
    ...
    func UpcommingRefName() {
        ...
        APiCallManager.shared.callApi(url: APIEndpoints.GetAssignmentListByType
                                      , httpMethod: .post, queryParam: nil, requestBody: Upcom) { [weak self] (result:Result<senderUpcommingResponces,Error>) in
            guard let self = self else{return}
            switch result{
            case .success(let success):
                if success.Status == 1 {
    ...
    func PastRefName() {
        ...
        APiCallManager.shared.callApi(url: APIEndpoints.GetAssignmentListByType
                                      , httpMethod: .post, queryParam: nil, requestBody: past) { [weak self] (result:Result<senderUpcommingResponces,Error>) in
            guard let self = self else{return}
            switch result{
            case .success(let success):
                if success.Status == 1{
