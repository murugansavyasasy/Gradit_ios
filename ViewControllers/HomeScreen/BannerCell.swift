...
class BannerCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        viewAllBtn.layer.borderColor = UIColor.tintColor.withAlphaComponent(0.13).cgColor
    ...
    private func updatePageControl() {
        ...
        let page = Int(round(cv.contentOffset.x / cv.bounds.width))
        let validPage = max(0,min(page, emergencyVoices.count - 1))
...
extension BannerCell: UICollectionViewDelegate,
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier:"EmergencyVoiceCV",
                                                      for: indexPath) as! EmergencyVoiceCV
        ...
        attributedString.addAttributes([.foregroundColor : UIColor.black,.font: UIFont.systemFont(ofSize: 13, weight: .regular)],range: NSRange(location: 0, length: "Posted by : ".count))
        attributedString.addAttributes(
            [.foregroundColor: colorForPriority() ?? .purple,.font: UIFont.systemFont(ofSize: 14, weight: .medium)],
        ...
        // MARK: - Date & Time
        cell.DateLbl.text = voice.createdon?.dateFormater(input: "dd/MM/yyyy hh:mm:ss a") ?? "--/--/----"
        cell.timeLbl.text = voice.createdon?.dateFormater(input: "dd/MM/yyyy hh:mm:ss a",output: "hh:mm a") ?? "--:--"
        ...
        cell.playAction = { [weak self, weak cell] in
            ...
            if let previousCell = self?.activeCell, previousCell !== cell {
                previousCell.playBtn.setImage(UIImage(systemName: "play.fill"), for: .normal)
...
final class AudioPlayerManager {
    ...
    private func addTimeObserver() {
        ...
        timeObserver = player?.addPeriodicTimeObserver(forInterval: interval,queue: .main){[weak self] time in
            ...
            self.onProgress?(progress,currentTime,duration)
        }
    }
