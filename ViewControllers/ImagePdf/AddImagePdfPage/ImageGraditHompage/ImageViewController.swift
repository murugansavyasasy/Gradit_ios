...
class ImageViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if imageSegmentName.selectedSegmentIndex == 0{
            ...
            let filtered_list : [departmentImageDataDetails] = cloneList
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if imageSegmentName.selectedSegmentIndex == 0 {
            ...
            let counts = image.newfilepath?.count ?? 0-1
            ...
            let cutss = parsed?.suffix(43) ?? ""
            let strss = String(cutss)
            let counts = images.newfilepath?.count ?? 0-1
            let srtcovert = String(counts)
    ...
    func departRefName() {
        ...
        APiCallManager.shared.callApi(url: APIEndpoints.GetCircularListByType, httpMethod: .post, queryParam: nil, requestBody: depart) { [weak self] (result:Result<departmentImageResponce,Error>)  in
